    ...
    class AddKey < Base
      ...
      def run(mail)
        ...
        if key_material.blank?
          mail.list.logger.debug 'Found no key material in message - sending error message'
          return t('no_content_found')
        end

        import_results = Array(key_material).map do |importable|
          keys_controller.import(mail.list.email, importable)
        end
        ...
        import_stati.each do |import_status|
          if import_status.action == 'error'
            ...
            key = mail.list.gpg.find_distinct_key(import_status.fingerprint)
            if key
