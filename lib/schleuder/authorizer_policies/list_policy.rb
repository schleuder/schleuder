module Schleuder
  module AuthorizerPolicies
    class ListPolicy < BasePolicy
      ...
      def resend_encrypted?
        superadmin? || admin?(resource) || subscriber_permitted?(resource, 'resend-encrypted')
      end

      def resend_unencrypted?
        superadmin? || admin?(resource) || subscriber_permitted?(resource, 'resend-unencrypted')
      end
    end
  end
end
