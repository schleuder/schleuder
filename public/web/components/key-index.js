...
import PlusSign from './plus-sign.js'
...
export default class KeyIndex extends BaseComp {
  constructor(listname) {
    super(listname);
    this.append(
      new ActionLink(this.urlFor(["keys", "new"]), [new PlusSign(), "Upload key"]),
      h1(`Keys known to ${listname}`)
    )
    Backend.fetch(listname, "keys")
      .then((keys) => {
        const cards = keys.map((key) => {
          const card = new IndexCard('key', this.linkTo(key.key_summary, ["keys", key.fingerprint]))
          if (key.subscription) {
            card.append(new CardPopup('person', div([
                  "Used by ",
                  this.linkTo(key.subscription, ["subscriptions", key.subscription])
                ])))
          }
          return card
        })
