...
describe Schleuder::Runner do
  describe "#run" do
    ...
    context 'protected headers' do
      it 'passes mails with protected headers and weird multipart' do
        list = create(:list)
        ...
        mail.gpg(gpg_opts)
        mail.body = File.read('spec/fixtures/mails/protected_headers.txt')
        mail.deliver

        protected_headers = Mail::TestMailer.deliveries.first
        Mail::TestMailer.deliveries.clear

        output = process_mail(protected_headers.to_s, list.email)
