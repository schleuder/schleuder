...
describe 'AddSigEncToHeadersToMetaDefaults' do
  let(:newest_migration) { 20190906194820 }
  let(:migration_under_test) { 20180110203100 }
  let(:previous_migration) { 20170713215059 }

  after(:each) do
    # How to run all migrations???
    ActiveRecord::Base.connection.migration_context.run(:up, newest_migration)
  ...
  describe 'up' do
    it 'sets the column defaults' do
      ActiveRecord::Base.connection.migration_context.run(:down, previous_migration)
      list_klass = create_list_klass
      list_klass.reset_column_information

      ActiveRecord::Base.connection.migration_context.run(:up, migration_under_test)
      list_klass.reset_column_information
